...
var getItem = function(term){  
  var item = vocab[term.toLowerCase()]  
  if (item){
    var detection = {
      type: item.type,
      value: item.key,
    ...
    var distanceItem = _.max(vocab, function(item){ 
...
exports.do = function (request, vocabLoader, callback) {
  vocab = vocabLoader.vocab;
  var result = {};
  result.detections = [];
  ...
  for (i = 0;i<request.tokens.length;i++){
    ...
    if (tokenTwo
      ...
      if (token && !token.stopWord && (token.pos[0] == "J" | token.pos[0] == "N" | token.pos[0] == "V")){
