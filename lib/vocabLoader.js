var 
  ...
  logger = log4js.getLogger('vocabLoader'),
...
var addVocabItem = function(item){
  if (vocab[item.key]){    
    logger.warn("app=detection,module=vocabLoader,function=addVocabItem,message=vocab conflict,newItemKey=%s,newItemType=%s,newItemCount=%d,existingItemKey=%s,existingItemType=%s,existingItemCount=%d", item.key, item.type, item.count, vocab[item.key].key, vocab[item.key].type, vocab[item.key].count  )
  } else {
    vocab[item.key] = item;
  }
};
  ...
  db.collection('shoes').mapReduce(
    ...
    function (err, results) {    
      if (err){
        ...
        logger.error("app=detection,function=populateColours,module=vocabLoader,resource=mongo,error=%s", JSON.stringify(err));
      } else{
        ...
        logger.info("app=detection,function=populateColours,module=vocabLoader,coloursVocab=loaded");      
  ...
  db.collection('shoes').mapReduce(
    ...
    function (err, results) {   
      if (err){
        ...
        logger.error("app=detection,function=populateMaterials,module=vocabLoader,resource=mongo,error=%s", JSON.stringify(err));
      } else{
        ...
        logger.info("app=detection,function=populateMaterials,module=vocabLoader,materialsVocab=loaded");
  ...
  db.collection('shoes').mapReduce(
    ...
    function (err, results) {   
      if (err){
        ...
        logger.error("app=detection,function=populateStyles,resource=mongo,error=%s", JSON.stringify(err));
      } else{
        ...
        logger.info("app=detection,function=populateStyles,module=vocabLoader,stylesVocab=loaded");
  ...
  db.collection('shoes').mapReduce(
    ...
    function (err, results) {   
      if (err){
        ...
        logger.error("app=detection,function=populateBrands,module=vocabLoader,resource=mongo,error=%s", JSON.stringify(err));
      } else{
        ...
        logger.info("app=detection,function=populateBrands,module=vocabLoader,brandsVocab=loaded");
...
exports.loadData = function(callback){
  mongoClient.connect(mongoUri, function (err, db) {
    if (err){
      logger.error("app=detection,function=loadData,module=vocabLoader,resource=mongo,error=%s", JSON.stringify(err));
      throw err;
    }else{    
      async.series([
        function(callback){
      ...
      function(err, results){
        if(err){
          ...
          exports.vocab = vocab; 
          db.close();
          logger.info("app=detection,function=loadData,module=vocabLoader,action=completed");
          callback();          
        }
      });
    }  
